...
const PUSH_TOKEN_STORAGE_KEY = "front-client:push:fcm-token";
...
async function registerPushToken(pushToken: string) {
  ...
  const defaultUrl = `/api/v1/preferences/devices/${deviceId}/push-token`;
  const url = appEnv.pushRegistrationUrl?.replace("{deviceId}", deviceId) ?? defaultUrl;
...
export async function initializeClientPushNotifications(
  ...
  if (!foregroundListenerBound) {
    onMessage(messaging, (payload: { notification?: { title?: string; body?: string } }) => {
      useNotificationStore
        .getState()
        .pushToast("info", payload.notification?.title ?? "Новая операция по вашему счёту");
  ...
  enqueueTelemetry({
    ...
    service: "firebase-push",
    state: "ready",
    message: "Firebase push initialized.",
  });
